...
export PATH=$PATH:/usr/j2se/current/bin
...
function usage() {
    ...
    echo "Usage: $prog -d <docker> [-w <%ratio>] [-c <%ratio>]";
    echo "       -p <pid>       the PID of process to monitor"
    echo "       -s <service>   the service name of process to monitor"
    echo "       -j <java-name> regexp to match against the full command line"
    echo "                      if this name in blank (-j '') any java app is"
    echo "                      looked for (as long there is only one)"
    echo "       -d <docker>    name of the docker with java application running"
    echo "       -u <user>      the user name whose process to monitor"
...
while getopts hvp:s:j:d:u:w:c: opt ; do
    case ${opt} in
    ...
    d)  docker_name="${OPTARG}"
        pid=1
        docker_cmd="docker exec ${docker_name}"
        ;;
    u)  user_name="${OPTARG}"
        ;;
    w)  ws="${OPTARG}"
...
#if [ -z "$pid" -a -z "$service" -a $use_jps -a -z "$docker_name" -eq 0 ] ; then
#    echo "One of -p, -s, -j or -d parameter must be provided"
#    usage $0
#    exit 3
#fi
...
if [ $use_jps -eq 1 ] ; then
    if [ -n "$java_name" ] ; then
        java=$(pgrep -u "$user_name" -f "$java_name" 2>/dev/null)
    else
...
gc=$(${docker_cmd} jstat -gc $pid | tail -1 | sed -e 's/[ ][ ]*/ /g')
if [ -z "$gc" ]; then
...
gccapacity=$(${docker_cmd} jstat -gccapacity $pid | tail -1 | sed -e 's/[ ][ ]*/ /g')
...
perfdata="heap=$heap heapmax=$heapmx perm=$pu permmax=$pgcmx"
